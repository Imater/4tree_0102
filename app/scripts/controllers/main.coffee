...
angular.module("4treeApp").controller "MainCtrl", [ '$translate', '$scope', ( $translate, $scope) ->
  ...
  $scope.text_example1 = "В третьей части «Незаметных сложностей ракетной техники» я бы хотел рассказать о ещё не охваченных в первой и второй частях проблемах, которые требуют решения. Эта статья посвящена развитию темы различных ограничений, которые приводят к инженерно неоптимальным решениям.
...
Третья — «Ариан-4». Семейство «Ариан» ведет своё начало от боевых ракет.
