...
function compare(path, old, new_, time) {
  var changes = [];
  if (old !== null && new_ !== null && typeof old === 'object') {
    ...
    sameKeys.forEach(function (k) {
      var childChanges = compare(path.concat(k), old[k], new_[k], time);
      changes = changes.concat(childChanges);
    });

    var delKeys = _.difference(oldKeys, newKeys);
    delKeys.forEach(function (k) {
      changes.push({ type: 'del', key: path.concat(k), tm: time});
    });

    var newKeys_ = _.difference(newKeys, oldKeys);
    newKeys_.forEach(function (k) {
      changes.push(delCheck(
        { type: 'put', key: path.concat(k), value: new_[k], tm: time }));
    });

  } else if (old !== new_) {
    changes.push(delCheck({ type: 'put', key: path, value: new_, tm: time }));
  }
